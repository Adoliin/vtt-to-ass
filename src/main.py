...
def main():
    ...
    writeSubFile(subPathAss, ''.join(fixed_linesAss))
    os.remove(tmp_subPathAss)
    print(f'Sub converted: {subPathAss}')

def getOpts():
    global subPathVtt, tmp_subPathAss, subPathAss
    try:
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            try:
                subPathVtt = os.path.abspath(arg)
            except:
            ...
            try:
                subPathAss = os.path.abspath(arg)
            except:
